...
---
# Форма
![[F_Auth-1777997548159.webp|302|543x187]]
## Назначение формы
- Аутентификация пользователей
## Элементы интерфейса

| Тип элемента  | Имя элемента | Caption/Text |
| ------------- | ------------ | ------------ |
| CommandButton | Btn_Add      | Войти        |
| CommandButton | Btn_Cansel   | Отмена       |
| Label         | Lbl_Login    | Логин        |
| Label         | Lbl_Password | Пароль       |
| TextBox       | TB_Login     |              |
| TextBox       | TB_Password  |              |
...
' @todo: Убрать неиспользуемые переменные sql/Check_login/userID/roleID/userLogin и добавить ShowError в обработчик ошибок.
    ...
    If ValidateUserPassword(Me.TB_Login.Value, Me.TB_Password.Value) Then
        Call SessionStartup(GetUserIdByLogin(Me.TB_Login.Value), Me.TB_Login.Value, GetUserRoleId(GetUserIdByLogin(Me.TB_Login.Value)))
        Unload Me
    Else
