...
// ===== Классы, от которых создаются объекты =====
...
// Берём только Private, не в событийных модулях, не в живых классах,
// и которые не находятся среди явных root/magic.
const deadCandidates = procs.filter(p => {
...
const header = dv.el("div", "Кандидаты на мёртвый код (Private)", { container: wrap });
header.style.cssText = `
  font-size: 0.95em;
  font-weight: 600;
  color: var(--text-normal);
...
const sub = dv.el(
  ...
  deadCandidates.length === 0
    ? "Под критерий (Private, не события, не классы с объектами) никто не попал."
    : `Private‑процедур/функций‑кандидатов: ${deadCandidates.length}`,
  { container: wrap }
);
sub.style.cssText = `
  font-size: 0.85em;
