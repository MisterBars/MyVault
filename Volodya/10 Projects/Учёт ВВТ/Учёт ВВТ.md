...
// типы страниц, которые считаем кодовыми
const CODE_TYPES = ["module", "class", "form"];

// формы считаем событийными по умолчанию
const EVENT_TYPES = ["form"];

// имена "магических" точек входа Excel/VBA
const MAGIC_ENTRY_NAMES = new Set([
  "Auto_Open",
  "Workbook_Open",
  "Workbook_BeforeClose",
  "Workbook_BeforeSave"
  // сюда можешь добавить свои
]);

// имена модулей, которые считаем событийными (ThisWorkbook, листы и т.п.)
const EVENT_MODULE_NAMES = new Set([
  "ThisWorkbook"
  // можно добавить шаблон для "Sheet1", "Лист1" и т.д., если хочешь
]);

// ручной список корневых процедур
...
const reProcDecl = /^\s*(?:(Public|Private)\s+)?(?:Static\s+)?(Sub|Function)\s+([A-Za-z0-9_]+)/i;
// Dim x As MyClass
const reDimAsClass = /Dim\s+\w+\s+As\s+([A-Za-z0-9_]+)/i;
// Set x = New MyClass
const reNewClass = /New\s+([A-Za-z0-9_]+)/i;

// структуры
let modulesText = [];
let procs = []; // { id, name, modulePath, moduleName, isEventModule, isClassModule, isMagicEntry }
let classes = new Set(); // имена классов
let classModulesByName = new Map(); // className -> modulePath[]
...
const procsByName = new Map(); // name -> [procIds…]
const procsById = new Map();   // id -> proc

for (const p of procs) {
  procsById.set(p.id, p);
  if (!procsByName.has(p.name)) procsByName.set(p.name, []);
  procsByName.get(p.name).push(p.id);
}

// ===== Строим граф вызовов по тексту =====

const calls = new Map(); // callerId -> Set(calleeId)
for (const p of procs) {
  calls.set(p.id, new Set());
}

// Грубый call graph: ищем упоминания имени процедуры как слова в тексте модуля.
for (const mt of modulesText) {
  const modulePath = mt.page.file.path;
  const text = mt.text;

  // список процедур этого модуля (как потенциальные caller)
  const moduleProcs = procs.filter(p => p.modulePath === modulePath);
  if (moduleProcs.length === 0) continue;

  for (const [name, ids] of procsByName.entries()) {
    const reWord = new RegExp(`\\b${name}\\b`, "i");
    if (!reWord.test(text)) continue;

    for (const caller of moduleProcs) {
      for (const calleeId of ids) {
        calls.get(caller.id).add(calleeId);
      }
    }
  }
}

// ===== Определяем классы, которые реально создаются =====
...
// 1) Все процедуры в EVENT_TYPES (формы, ThisWorkbook и т.п.)
for (const p of procs) {
  if (p.isEventModule) {
    alive.add(p.id);
  }
...
// 2) Все "магические" точки входа (Auto_Open и т.п.)
for (const p of procs) {
  if (p.isMagicEntry) {
    alive.add(p.id);
  }
...
// 3) Корневые процедуры из ROOT_PROCS
for (const p of procs) {
...
// 4) Все методы классов, которые используются (создаются объекты класса)
for (const className of liveClasses) {
  const modulePaths = classModulesByName.get(className) || [];
  for (const p of procs) {
    if (modulePaths.includes(p.modulePath)) {
      alive.add(p.id);
    }
  }
}

// 5) Распространяем "живость" по графу вызовов
const queue = [...alive];
