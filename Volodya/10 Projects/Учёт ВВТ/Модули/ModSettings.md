...
' Настройки
Public ID_table_menu, ID_table_lists, ID_table_change, ID_LB_Line As Integer ' ID для мультипейджа
Public ActionBut As String  ' ID для мультипейджа формы редактирования
Public Const ADDIN_PATH As String = "D:\ДСП\Учёт_ВВТ\" ' не забыть потом сделать именно там где запускается проект(изначально предполагается такой путь)
Public Const SERVER_BASE As String = "" ' Путь к БД на сервере
Public Const SERVER_DOC As String = "" ' Путь к БД на сервере
Public Const LOCAL_BASE As String = "D:\ДСП\Учёт_ВВТ\vvt_db.accdb" ' Путь к БД на ПК
Public Const LOCAL_DOC As String = "D:\ДСП\Учёт_ВВТ\Документы\" ' Путь к БД на ПК
Public Const SRV_LOC As Boolean = False

Public Function GetConstant(constantName As String) As Variant
' @desc: Возвращает значение одной из констант настроек по её имени.
' @role: Config
' @todo: Добавить новые константы проекта и при необходимости логирование обращения к неизвестному имени.
End Function
    On Error GoTo ErrorHandler
    
    Select Case UCase(constantName)
        Case "ADDIN_PATH"
            GetConstant = ADDIN_PATH
        Case "SERVER_BASE"
            GetConstant = SERVER_BASE
        Case "SERVER_DOC"
            GetConstant = SERVER_DOC
        Case "LOCAL_BASE"
            GetConstant = LOCAL_BASE
        Case "LOCAL_DOC"
            GetConstant = LOCAL_DOC
        Case "SRV_LOC"
            GetConstant = SRV_LOC
        Case Else
            GetConstant = CVErr(xlErrNA)
    End Select
    
    Exit Function
    
ErrorHandler:
    GetConstant = CVErr(xlErrValue)
End Function
```
