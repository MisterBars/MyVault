...
# Модуль
# Назначение
Модуль реализует паттерн **"сессионный буфер"** — аналог ViewModel/ChangeTracker. Он полностью изолирует форму от БД во время редактирования: пользователь работает только с буфером, а реальная запись в `UserServices` происходит один раз при сохранении через `SaveServiceUsersLinks`. `NormalizeServiceUsersBufferAfterSave` закрывает цикл, синхронизируя буфер без повторного запроса.

## Важные решения
- In-memory буфер пользователей службы для F_Change
...
' @desc: Инициализирует пустой буфер пользователей службы. 
' @role: Init 
' @todo: --
...
' @desc: Полностью очищает буфер пользователей службы. 
' @role: Cleanup 
' @todo: Вызывать при закрытии формы.
    Set mItems = Nothing
End Sub

Public Function SvcBuf_IsReady() As Boolean
' @desc: Проверяет, инициализирован ли буфер. 
' @role: Service 
' @todo: --
...
Private Function NewUserState( _
    ...
    ' @desc: Создаёт объект состояния одного пользователя для буфера. 
    ' @role: Factory 
    ' @todo: Можно заменить Dictionary на Class.
    Dim d As Object
...
' @desc: Формирует ключ коллекции по идентификатору пользователя. 
' @role: Service 
' @todo: --
...
' @desc: Проверяет наличие пользователя в буфере по UserID. 
' @role: Query 
' @todo: --
    Dim obj As Object
...
' @desc: Возвращает объект пользователя из буфера по UserID. 
' @role: Query 
' @todo: --
    On Error Resume Next
...
Public Sub SvcBuf_AddOrReplace( _
    ...
    ' @desc: Добавляет пользователя в буфер или заменяет его состояние. 
    ' @role: Sync 
    ' @todo: --
...
' @desc: Загружает буфер из выборки пользователей существующей службы. 
' @role: Mapper 
' @todo: Проверять состав полей rs.
    Dim userID As Long
    Dim login As String
...
' @desc: Загружает в буфер пользователей с правами по роли для новой службы. 
' @role: Mapper 
' @todo: --
    Dim userID As Long
    Dim login As String
...
' @desc: Считает пользователей буфера, не помеченных на удаление. 
' @role: Calc 
' @todo: --
    Dim i As Long
    Dim obj As Object
...
' @desc: Определяет, можно ли показать пользователя в левом списке формы. 
' @role: UI 
' @todo: --
    Dim obj As Object
...
Public Function SvcBuf_AddLink( _
    ...
    ' @desc: Добавляет или восстанавливает связь пользователя со службой в буфере. 
    ' @role: Sync 
    ' @todo: --
...
' @desc: Помечает связь на удаление или удаляет нового пользователя из буфера. 
' @role: Sync 
' @todo: --
    Dim obj As Object
    Dim key As String
...
' @desc: Переключает флаг права редактирования у пользователя в буфере. 
' @role: Security 
' @todo: --
    Dim obj As Object
    Set obj = SvcBuf_Get(userID)
...
' @desc: Переключает флаг права согласования у пользователя в буфере. 
' @role: Security 
' @todo: --
    Dim obj As Object
    Set obj = SvcBuf_Get(userID)
...
' @desc: Возвращает коллекцию всех элементов буфера. 
' @role: Service 
' @todo: Ограничить прямое внешнее изменение при необходимости.
    Set SvcBuf_Items = mItems
End Function

Public Sub NormalizeServiceUsersBufferAfterSave()
' @desc: Нормализует буфер после сохранения изменений в базу.
' @role: Sync 
' @todo: Перепроверить логику для RightsByRole=False.
