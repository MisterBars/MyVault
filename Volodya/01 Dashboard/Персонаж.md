...
const lvls = "0,100,250,500,900,1400,2000,2700,3500,4500".split(",").map(Number);
let level = 1;
for (let i = 0; i < lvls.length - 1; i++) {
  if (totalXP >= lvls[i]) level = i + 1;
}

const xpA = lvls[level - 1] || 0;
const xpB = lvls[level] || 6000;
const pct = Math.max(0, Math.min(100, Math.round(((totalXP - xpA) / (xpB - xpA)) * 100)));

const ranks = ["","Новичок","Ученик","Специалист","Эксперт","Мастер","Архимаг","Легенда"];
...
dv.el("div", "🏆 Уровень " + level + " · " + totalXP + " XP", {container: wrap});
...
const inf = dv.el("div", (totalXP - xpA) + " / " + (xpB - xpA) + " XP до уровня " + (level + 1), {container: wrap});
...
for (const s of skillNotes) {
  ...
  const pct = Math.min(100, Math.round((xp / 750) * 100));
  ...
  dv.el("span", (s.title || s.file.name) + " · Ур. " + level, {container: lbl});
