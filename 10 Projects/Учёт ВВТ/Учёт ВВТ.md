...
for (const mt of modulesText) {
  ...
  reDimAsClass.lastIndex = 0;
  reNewClass.lastIndex = 0;
...
// === Быстрый фильтр кандидатов (Private) ===
...
// === Умная проверка: где встречается имя ===
function findNameUsageModules(procName) {
  ...
  for (const mt of modulesText) {
    ...
    if (reWord.test(mt.text)) {
      usedIn.add(mt.page.file.name);
    }
  }

  return Array.from(usedIn).sort();
}

const trulyDead = [];
const referencedButFlagged = [];

for (const p of candidates) {
  const usedInModules = findNameUsageModules(p.name);

  if (usedInModules.length === 0) {
    trulyDead.push(p);
  } else {
    referencedButFlagged.push({
      name: p.name,
...
const summary = dv.el(
  "div",
  `Всего Private‑кандидатов: ${candidates.length}. Точно без упоминаний: ${trulyDead.length}. Есть упоминания в коде: ${referencedButFlagged.length}.`,
  { container: wrap }
);
...
if (referencedButFlagged.length > 0) {
  ...
  for (const item of referencedButFlagged) {
    ...
    const left = dv.el("div", item.name, { container: row });
    left.style.cssText = `
      font-weight: 500;
      color: var(--text-normal);
      flex: 0 0 auto;
    ...
    const right = dv.el(
      "div",
      `${item.moduleName} ← упомянута в: ${item.usedInModules.join(", ")}`,
      { container: row }
    );
    right.style.cssText = `
      color: var(--text-muted);
...
## Важные файлы
1. [[10 Projects/Учёт ВВТ/Схема проекта|Схема]]
2. [[10 Projects/Учёт ВВТ/Прогресс проекта|Прогресс выполнения]]
