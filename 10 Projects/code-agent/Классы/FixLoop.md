...
## Назначение класса
Петля автоисправления ошибок.
Получает задачу, вызывает `OllamaClient.ask()` -> `CodeRunner.run()`.
Если в stderr есть ошибка - отправляет её обратно в LLM с просьбой исправить.
Повторяет до `max_retries` раз. Возвращает финальный результат.
