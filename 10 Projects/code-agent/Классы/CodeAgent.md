...
## Назначение класса
Legacy / v1 фасад ранней версии проекта.

Этот класс отражает первую итерацию Code Agent, где один фасад собирал `OllamaClient`, `CodeRunner` и `FixLoop` воедино и предоставлял единый метод `solve(task)`.

Сейчас этот класс больше не считается целевой архитектурой проекта.
Он сохраняется как:
- историческая основа v1
- возможный переходный слой
- foundation для worker-oriented execution flow
## Важные решения
- В v1 класс был центральной точкой входа для решения задач
- В v2 центральная роль переходит к `Orchestrator`
- `CodeAgent` можно сохранить как compatibility facade или CLI wrapper
- Основная multi-agent логика должна жить не здесь, а в `PlannerAgent`, `WorkerAgent`, `Orchestrator`
## Задачи по модулю
...
## Черновые заметки
### Новая роль в архитектуре
CodeAgent больше не главный “мозг” проекта.
Если файл останется в коде, то лучше использовать его как:
- фасад для CLI
- точку сборки зависимостей
- обёртку над Orchestrator